...
Well, this is my first blog post in this, and we're gonna look at a malware analysis challenge from the website [malops](https://malops.io/).

Rather than providing the answers to the questions in the website, I shall just show how I approached the challenge (following the structure of the questions). I am not experienced with Windows malware, so I shall be learning as I go through the challenge.

Alright, let's get started!

## Question 1: `Which version of the Go compiler was used to build this binary?`

Alright, so a `go` compiled binary, nothing that I haven't seen before. Let's just run `strings` on the binary to see if we find anything useful. A whole lot of strings; this is due to the fact that Go binaries are statically linked, so the binaries don't have any runtime dependencies, and the system does not require any loaders. Other than that, stuff like the Go runtime, std lib parts, metadata, and debug info are also present in the binary. Due to this, even small binaries have very large sizes.
...
Opening the binary, we instantly see that it's a stripped binary, and we do not have any symbols for it. Now we have more tools/scripts for IDA which can be used to recover a lot of the symbols (which we will look into later). Currently, we're just gonna see how to solve this normally without using external scripts.

Now since Go uses its own scheduler, its startup is quite distinct from `C/C++` startups. Your actual main is `main.main`. Before this is executed, there is an initialization routine which happens (this is pretty much the same for almost all Go binaries). This can be classified into 5 steps.
...
* `runtime_args_0` - In this, the `argv` and `argc` passed to the binary are moved from the OS memory to the Go memory.
...
Searching for `isRunningAsAdmin` we get this function `main_isRunningAsAdmin`, wherein there is a function call which we know is the solution for that question. So, what is `API hashing`?
...
Well for this one, I tried going through it from top to bottom, but it was taking a lot of time. So I went for a CTF styled approach, which was to search for the string (bytes) in IDA. !alt text
And according to the question, the last function *sounds* the most probable approach. So I'm gonna go with that. That's half the fun of solving challenges.
...
Alright so looking through `main.main` you can see the occurrence of the function `HCWin_etwevasion___ETWEvader__Enable`. The question just asks for the package, so it's pretty evident from the name itself what those are. 
...
For this one, I remember while going through the `main_encryptNetworkShares_func2` function, seeing the `main_findFiles_func1` which by chance had the "Skip %s..." string. So I went back to it.
Inside the else case there were 3 types of files/folders to skip or to accept. 
1) certain directories - `0x00000000010664A0`
2) Certain file names - `00000000010606A0`
3) Certain extensions (this is what is allowed, I guess) - `00000000010606C0` 
...
This is done for files larger than 500MB as shown in this command
`cmp     rbx, 1F400000h`
...
`wscript.exe //B //Nologo "%s"` is the execution command, `//B` suppresses all command errors and `//Nologo:` Prevents the WScript banner from displaying. 
